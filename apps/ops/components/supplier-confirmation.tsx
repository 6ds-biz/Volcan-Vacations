...
export function SupplierConfirmation({booking, reload}: {booking: Booking; reload: () => void}) {
  ...
  return <section className="ops-confirmation" aria-labelledby="supplier-confirmation-title"><h2 id="supplier-confirmation-title">Availability &amp; Supplier Confirmation</h2>
    ...
    </section><section><h3>Supplier for this request</h3><p><strong>{booking.supplier.name}</strong>{!booking.supplier.active && ' (inactive)'}<br />{booking.supplier.contact_name || 'No contact name'}<br />{booking.supplier.email || 'No email recorded'}<br />{booking.supplier.phone || 'No phone recorded'}</p><ContactActions email={booking.supplier.email} phone={booking.supplier.phone}/><p>Latest contact: {timeLabel(booking.supplier_contacted_at)}</p><p>Confirmation received: {timeLabel(booking.supplier_confirmed_at)}</p><p>Reference: <strong>{booking.supplier_confirmation_reference || 'None recorded'}</strong></p>{booking.supplier_response_notes && <p className="ops-preserve-text">{booking.supplier_response_notes}</p>}</section></div>
    ...
    <details className="supplier-history"><summary>Supplier contact timeline</summary>{!booking.supplier_events.length ? <p>No supplier actions recorded.</p> : <ol className="ops-timeline">{booking.supplier_events.map(event => <li key={event.id}><strong>{statusLabel(event.event_type)}</strong> · {timeLabel(event.occurred_at)}{event.contact_method && ` · ${event.contact_method === 'whatsapp' ? 'WhatsApp' : statusLabel(event.contact_method)}`}{event.actor_display_name ? ` · by ${event.actor_display_name}` : event.operator_identifier ? ` · ${event.operator_identifier}` : ' · System / historical record'}<p>Reservation: {statusLabel(event.reservation_status)} · Availability: {statusLabel(event.availability_status)} · Supplier: {statusLabel(event.status)}</p>{event.reference && <p>Reference: {event.reference}</p>}{(event.alternative_tour_name || event.alternative_date || event.alternative_time) && <p>Proposed alternative: {event.alternative_tour_name || booking.tour_name} · {event.alternative_date || 'Date to discuss'} · {event.alternative_time || 'Time to discuss'}</p>}{event.notes && <p className="ops-preserve-text">{event.notes}</p>}<small>Recorded {timeLabel(event.created_at)}</small></li>)}</ol>}</details>
  </section>;
}
