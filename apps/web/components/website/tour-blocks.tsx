...
export function TourBlock({kind,tour,config}:{kind:string;tour:Tour;config:Editorial}){
 ...
 if(kind==='tour-hero')return <section className="website-tour-hero"><div className="website-tour-photo">{config.media?<ConfiguredMedia config={config.media} scenic/>:<TourPhoto image={tour.primary_image}/>}</div><Container><p className="eyebrow">{tour.location||'Costa Rica'}</p><h1>{tour.name}</h1></Container></section>;
 ...
 gallery:tour.images.length>0&&<><h2>{config.heading||'Gallery'}</h2><div className="website-tour-gallery">{tour.images.map(image=><div key={image.id}><TourPhoto image={image}/></div>)}</div></>,
 ...
 return content[kind]?<section className="section"><Container>{content[kind]}</Container></section>:null;
}
